...
- `profiles` — one row per Supabase auth user (`id` = auth user id). Holds `role`/`area` plus the full volunteer application captured by `Register.tsx` (contact info, emergency contact, study center, medical conditions, shirt size, geolocation) and displayed in `VolunteerList.tsx`. Also holds `qr_token`, the value encoded into a volunteer's QR credential (Dashboard's "Mi Código QR" tab, via `react-qr-code`).
...
- `marketing_tasks` — content-calendar items for the `"Comunicación y Difusión"` area (`MarketingBoard.tsx`), with status pipeline `Pendiente → En Revisión → Observado/Publicado`; `draft_date` is auto-derived client-side as `post_date - 7 days`. `references` (max 3, enforced by a DB check constraint) is a reserved SQL keyword — always double-quote it (`"references"`) in raw SQL. `submission_link` is filled in by the assignee via a modal when they click "Enviar a Revisión" (not by the supervisor at task-creation time). `extension_days` tracks how many times a supervisor pushed out `draft_date` via the "Extender +1 día" action. `is_active` is a soft-delete flag — the board's fetch always filters `eq('is_active', true)`, and "Archivar" just flips it to `false` rather than deleting the row.
